...
**정모 · 추가정모 · 킥키타카 · SUFA · 친선 · 기타 대회** 시간과 장소를 바로 보고, 다가오는 경기·대회도 같은 화면에서 확인합니다. 마스코트는 **엘펜이**입니다.
...
| 경로 | 화면 |
|------|------|
| `/` | 오늘/다음 카드 + 엘펜이(말풍선) + 갱신 시각 + 다가오는 경기·대회 |
| `/calendar` | 한 달 달력. 뒤에 로고 워터마크, 오늘은 날짜에 주황 원. 칸에는 종류·시간·상대 |
| `/tournaments` | 다가오는 킥키타카·SUFA·친선·기타 대회 목록 |
| `/events/:id` | 그 일정의 시간·장소·상대·도착 시각 |
| `/edit` | 임원진만. 들어올 때마다 운영 비밀번호 |

달력에서 날짜를 누르면 상세가 열립니다. 종류는 색 타원 뱃지로만 보여 주고, 정모는 큰 제목을 쓰지 않습니다. 킥키타카·SUFA·친선·기타 대회는 `vs 상대`와 경기 시작 **1시간 30분 전** 도착 시각을 넣습니다. 장소는 달력 칸이 아니라 상세에서 봅니다. 달력을 스크롤해도 위 헤더와 겹치지 않습니다.
...
3. 종류(정모 / 추가정모 / 킥키타카 / SUFA / 친선 / 기타 대회), 날짜, 시간, 장소를 넣습니다. 제목은 없습니다. 킥키타카·친선은 **상대**를, SUFA·기타 대회는 필요하면 **상대**를 넣습니다.
